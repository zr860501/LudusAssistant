...
python3 - "$OCR" <<'PY'
from pathlib import Path
p=Path(__import__('sys').argv[1]); s=p.read_text().replace('android.graphics.ImageFormat.RGBA_8888','android.graphics.PixelFormat.RGBA_8888'); p.write_text(s)
PY

# Replace EventStore with a compile-safe implementation. The prior helper used
# optString(), whose checked-exception signature varies across Android JSON stubs.
...
if [ -n "${EVENT:-}" ] && [ -f "$EVENT" ]; then grep -q 'event.getString("id")' "$EVENT"; fi
